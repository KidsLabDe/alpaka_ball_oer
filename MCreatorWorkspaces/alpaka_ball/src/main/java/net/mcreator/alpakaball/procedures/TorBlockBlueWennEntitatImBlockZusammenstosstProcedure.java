...
public class TorBlockBlueWennEntitatImBlockZusammenstosstProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		if ((entity.getDisplayName().getString()).equals("Flugball") && !entity.getPersistentData().getBoolean("im_tor")) {
			...
			AlpakaBallModVariables.MapVariables.get(world).punktzahl_blau = HaecksenBallModVariables.MapVariables.get(world).punktzahl_blau + 1;
			AlpakaBallModVariables.MapVariables.get(world).syncData(world);
			if (world instanceof ServerLevel _level)
				_level.getServer().getCommands().performPrefixedCommand(new CommandSourceStack(CommandSource.NULL, new Vec3(x, y, z), Vec2.ZERO, _level, 4, "", Component.literal(""), _level.getServer(), null).withSuppressedOutput(),
						("/bossbar set blue value " + Math.round(AlpakaBallModVariables.MapVariables.get(world).punktzahl_blau)));
			if (world instanceof ServerLevel _level)
				_level.getServer().getCommands().performPrefixedCommand(new CommandSourceStack(CommandSource.NULL, new Vec3(x, y, z), Vec2.ZERO, _level, 4, "", Component.literal(""), _level.getServer(), null).withSuppressedOutput(),
